...
def profile():
    ...
    user_images = db.session.query(
        UserImages.user_id, UserImages.timestamp, UserImages.image_uuid,
        UserImages.annotation, Images.thumbnail_url).filter(
            Images.uuid == UserImages.image_uuid).filter(
                UserImages.user_id == current_user.id).order_by(
                    UserImages.timestamp.asc()).all()
...
def new_image():
    ...
    if current_user.is_authenticated:
        completed_images = {
            x[0]
            for x in db.session.query(UserImages.image_uuid).filter(
                UserImages.user_id == current_user.id).all()
        }

        image_uuid = random.sample(all_images.difference(completed_images),
                                   1)[0]
    else:
